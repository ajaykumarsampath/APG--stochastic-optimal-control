function [ yalmip_tree,yalmip_const_time] = yalmip_primal_multiple(sys,V,Tree,ops)
%This a that creates an yalmip optimizer object to solve the optimization
%on a scenario tree. 
%
% Syntax:  [ yalmip_tree,yalmip_const_time] = yalmip_primal_differentFt(sys,V,Tree,ops)
% 
...
if nargin==4, % User-provided options
    sdp_options = ops;
...
for i=1:Tree.ancestor(Tree.leaves(end))
    ...
    const_yal=const_yal+(sys.F*Xyal(:,i)+sys.G*Uyal(:,i)<=sys.g);
...
for i=1:Ns
    ...
    J_yal=J_yal+pl*(Xyal(:,Tree.leaves(i))'*V.Vf{i}*Xyal(:,Tree.leaves(i)));
      const_yal=const_yal+(sys.Ft{i}*Xyal(:,Tree.leaves(i))<=sys.gt{i});
end
details.yalmip_const=toc;
% building a optimizer 
%sdp_options = sdpsettings('solver','gurobi','verbose',0,'cachesolvers',1);
tic
yalmip_tree=optimizer(const_yal,J_yal,sdp_options,{Xinit},{Xyal,Uyal,J_yal});
